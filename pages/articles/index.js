   ...
   const hanldeChange = (definition, value) => {
      if (definition !== 'page' && definition !== 'limit') {
         ...
         if (Array.isArray(router.query[definition])) {
            ...
            const index = params.indexOf(value.toString());3
            if (index === -1) {
                  ...
                  <div className={styles.filterBy}>
                     ...
                     {(categories && categories.data) ? (
                        ...
                        <div className={`${styles.wrap} ${styles.xer}`}>
                           <span className={activeContent === 1 ? `${styles.ttl}, ${styles.activeTtl}` : styles.ttl}
                             onClick={() => activeTab(1)}>Categories<span className={styles.slush}>/</span></span>
                        </div>
                        <div className={activeContent === 1 ? `${styles.list}, ${styles.activeList}` : styles.list}>
                              {categories.data.map((cat) => {
                                 return (
                                    <div className={`${styles.item}, cornered-cat-item ${isActive('category', cat.id) ? styles.active : ''}`}
                     ...
                     {(levels && levels.data) && (
                        ...
                        <div className={styles.wrap}>
                           <span className={activeContent === 2 ? `${styles.ttl}, ${styles.activeTtl}` : styles.ttl}
                             onClick={() => activeTab(2)}>Difficulty<span className={styles.slush}>/</span></span>
                        </div>
                        <div className={activeContent === 2 ? `${styles.list}, ${styles.activeList}` : styles.list}>
                              {levels.data.map((lev) => {
                                 return (
                                    <div
                                       className={`${styles.item}, lev-item ${lev.title.toLowerCase()} 
                                       ${isActive('level', lev.id) ? styles.active : ''}`}
                     ...
                     {(tags && tags.data) && (
                        <div>
                        <div className={styles.wrap}>
                           <span className={activeContent === 3 ? `${styles.ttl}, ${styles.activeTtl}` : styles.ttl}
                             onClick={() => activeTab(3)}>Tags</span>
                        </div>

                        <div className={activeContent === 3 ? `${styles.list}, ${styles.activeList}` : styles.list}>
                              {tags.data.map((tag) => {
                                 return (<div
                                    className={`${styles.item}, tag-item ${isActive('tag', tag.id) ? styles.active : ''}`}
